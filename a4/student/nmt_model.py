...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        #permute와 pack_padded_sequence의 input 및 output = ? 
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        #padding과 packing의 차이? https://velog.io/@uvictoli/%EB%AA%A8%EB%91%90%EB%A5%BC-%EC%9C%84%ED%95%9C-%EB%94%A5%EB%9F%AC%EB%8B%9D2-RNN-6-Packed-Sequence 참조. 
        #언더바는 왜 있는데? 그냥 더미데이터 처리용?
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        #bidirectional = True로 설정한 encoder layer의 결과물이 이미 양방향 state의 concatenation된 값인데 왜 여기서 또 torch.cat을 실행함?
