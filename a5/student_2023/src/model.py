...
class DownProjectBlock(nn.Module):
    ...
    def forward(self, x_input):
        ...
        # x = x_input + self.attn(self.ln1(x_input), self.ln1(self.C)) 아님? 그림에 따르면 여기도 add norm 해야하는데 왜 여기서는 residual connection 안 함?
        x = x + self.mlp(self.ln2(x))
        return x
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        #여기도 마찬가지, 왜 residual connection 안함? x_input이 이미 residual을 포함한 값인가?
        #그렇게 따지기엔 그림에서 한 쪽은 x_input을 그대로 더하고, 한 쪽은 x_input에다가 attention 처리를 함.
        #혹은 차원 문제?
        x = x + self.mlp(self.ln2(x))
        return x
