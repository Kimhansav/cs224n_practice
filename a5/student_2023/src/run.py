...
if args.variant == 'vanilla':
    ...
    model = model.GPT(mconf).to(device) #after you load the model, it is no longer on GPU so add .to(device) GPU가 병렬연산에 특화되어 딥러닝에 활용됨.
...
if args.function == 'pretrain':
    ...
    if args.reading_params_path is not None: #finetuning with pretrained model
    ...
    else: #finetuning without pretrained model
        trainer_config = trainer.TrainerConfig(
            max_epochs=75,
            batch_size=256,
            learning_rate=args.finetune_lr,
